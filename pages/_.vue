<template>
    <div class="flex flex-wrap-reverse" :class="{ 'lg:-mx-8': settings.layout === 'single' }">
        <div class="w-full py-4 lg:pt-8 lg:pb-4 dark:border-gray-800"
             :class="{ 'lg:w-3/4': !document.fullscreen, 'lg:border-l lg:border-r': settings.layout !== 'single'}">
...
<script>
    ...
    export default {
        ...
        async asyncData({$content, store, app, params, error}) {
            ...
            let otherData = await $content(app.i18n.locale, {deep: true})
                .surround(document.slug, {before: 1, after: 1})
                .only(['title', 'slug', 'to'])
                .sortBy('position', 'asc')
                .fetch()
